				       ...
				       std::string const &Message, bool const errTransient, bool const errAuthErr)
{
   ...
   for (auto const Owner : ItmOwners)
   {
	 ...
	 if (_config->FindB("Acquire::Retries::Delay", true))
	 {
	    ...
	    auto const retry_after_str = LookupTag(Message, "Retry-After");
	    auto const fail_reason = LookupTag(Message, "FailReason");
	    if (fail_reason == "TooManyRequests" && !retry_after_str.empty() && HandleRetryAfter)
	    {
	       // The webserver gave a retry time. Use it, but also add exponential
	       // backoff waiting as we might not be the only client.
	       const auto retry_after = std::chrono::seconds(std::strtoul(retry_after_str.c_str(), nullptr, 10));
	       const auto epoch_current = std::chrono::duration_cast<std::chrono::seconds>(currentWall.time_since_epoch());
	       // If the retry_after is in the past, we can just continue.
	       if (retry_after > epoch_current)
		  Dur += retry_after - epoch_current;
